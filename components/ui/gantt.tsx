...
import { addDays, addMonths, differenceInDays, differenceInHours, differenceInMonths, endOfDay, endOfMonth, getDaysInMonth, startOfDay, startOfMonth } from 'date-fns'
...
export const GanttContentHeader: FC<GanttContentHeaderProps> = ({ title, columns, renderHeaderItem }) => {
    ...
    return (
        <div
            ...
            <div>
                <div
                    className="text-muted-foreground sticky inline-flex whitespace-nowrap px-3 py-2 text-xs"
                    style={{
...
export const GanttSidebar: FC<GanttSidebarProps> = ({ children, className }) => (
    <div
        ...
        className={cn('border-border/50 bg-background/90 sticky left-0 z-30 h-max min-h-full overflow-clip border-r backdrop-blur-md', className)}>
        <GanttSidebarHeader />
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({ onCreateMarker, className }) => {
    ...
    return (
        <div
            className={cn('group pointer-events-none absolute left-0 top-0 h-full w-full select-none overflow-visible', className)}
            ref={mouseRef}>
            <div
                ...
                <div className="border-border/50 bg-background/90 text-foreground whitespace-nowrap rounded-full border px-2 py-1 text-xs backdrop-blur-lg">{formatDate(date, 'MMM dd, yyyy')}</div>
            </div>
        </div>
    )
}
...
export const GanttFeatureDragHelper: FC<GanttFeatureDragHelperProps> = ({ direction, featureId, date }) => {
    ...
    return (
        <div
            ...
            {...attributes}
            {...listeners}>
            <div className={cn('bg-muted-foreground absolute top-1/2 h-[80%] w-1 -translate-y-1/2 rounded-sm opacity-0 transition-all', direction === 'left' ? 'left-2.5' : 'right-2.5', direction === 'left' ? 'group-hover:left-0' : 'group-hover:right-0', isPressed && (direction === 'left' ? 'left-0' : 'right-0'), 'group-hover:opacity-100', isPressed && 'opacity-100')} />
            {date && <div className={cn('border-border/50 bg-background/90 text-foreground absolute top-10 hidden -translate-x-1/2 whitespace-nowrap rounded-lg border px-2 py-1 text-xs backdrop-blur-lg group-hover:block', isPressed && 'block')}>{format(date, 'MMM dd, yyyy')}</div>}
        </div>
    )
}
...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({ id, children }) => {
    ...
    return (
        <Card className="bg-background ring-foreground/10 h-full w-full rounded-md border-transparent p-2 text-xs shadow-sm ring-1">
            <div
        ...
        <div
            ...
            <ContextMenu>
                <ContextMenuTrigger asChild>
                    <div className={cn('bg-card text-foreground group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-b-md px-2 py-1 text-xs', className)}>
                        {label}
...
export const GanttProvider: FC<GanttProviderProps> = ({ zoom = 100, range = 'monthly', onAddItem, children, className }) => {
    ...
    return (
        <GanttContext.Provider
            ...
            <div
                className={cn('gantt bg-secondary relative grid h-full w-full flex-none select-none overflow-auto rounded-sm', range, className)}
                style={{
                    ...cssVariables,
